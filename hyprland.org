...
* Install
#+begin_src sh
yay -S kitty hyprland xdg-desktop-portal-hyprland xdg-desktop-portal-gtk polkit-kde-agent qt6-wayland hyprpaper gtklock
#+end_src
...
bind = $mainMod, Q, exec, hyprctl keywoard general:layout "dwindle"
bind = $mainMod SHIFT, Q, exec, hyprctl keywoard general:layout "master"
...
# TODO
bind = ,Print, exec, gtklock

bind = $mainMod, M, exec, wpctl set-mute @DEFAULT_AUDIO_SOURCE@ toggle
bind = $mainMod, P, exec, gopass --nosync ls --flat | rofi -dmenu -p 'Password for:' | xargs --no-run-if-empty gopass show -o | ydotool type --file=-

bind = $mainMod, U, exec, gopass --nosync ls --flat | rofi -dmenu -p 'User for:' | xargs -I "{}" --no-run-if-empty gopass show '{}' user | ydotool type --file=-

bind = $mainMod, O, exec, gopass --nosync ls --flat otp | rofi -dmenu -p 'OTP for:' | xargs --no-run-if-empty gopass otp | sed 's/ .*//' | ydotool type --file=-
