...
And this is last version of my config which is attemtp to use /Guix home/ for package installation and part of configuration.
...
alias m="mise"
alias mr="mise run"
alias mrt="mise --env test run"
alias mt="mise --env test"
...
But than it got discontinued, I switched to simpler configuration with Antigen.

And now I'm trying to use only packages present in Guix, let's see.
...
time to copy/adjust short description for most options I'm using, so I know what
it is doing.
...
When in normal mode, hit =e= to edit current command line in your ~$VISUAL~ (or
~$EDITOR~).
...
** Antidote - removing (it is great, but we have Guix)
...
**** zsh-autosuggestions - guixed :ARCHIVE:
...
**** TODO zsh-abbr - auto expanding abbreviations :ARCHIVE:
...
#+end_src
*** Nodenv - Node version manager :ARCHIVE:
...
I switched to mise everywhere. Currently installed from extra/mise in arch
*** PNPM
#+begin_src conf
...
*** Rbenv - Ruby version manager :ARCHIVE:
#+begin_src conf
...
*** ASDF :ARCHIVE:
I'm trying Elixir and Erlang and I found [[https://github.com/asdf-vm/asdf][GitHub - asdf-vm/asdf: Extendable
version manager with support for Ruby, Node...]]. It might be worth checking into
more plugins and use it instead of rbenv, nvm, pyenv etc.

Installed with ~yay SDf-vm~
...
**** Find latest version of something
#+begin_src sh
asdf latest elixir
#+end_src
